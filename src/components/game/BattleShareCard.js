...
export function createShareCard({ playerName, enemyName, playerWon, rounds, playerDmg, enemyDmg, crits, dodges }) {
    const safePlayerName = escapeHtml(String(playerName || 'Unknown Fighter'));
    const safeEnemyName = escapeHtml(String(enemyName || 'Unknown Opponent'));
    const resultText = playerWon ? 'VICTORY' : 'DEFEAT';
    const resultColor = playerWon ? '#10b981' : '#ef4444';
    const resultIcon = playerWon
        ? renderIcon('TROPHY', 'w-10 h-10 text-yellow-500 mx-auto')
        : renderIcon('SKULL', 'w-10 h-10 text-red-500 mx-auto');
        ...
        <div id="battle-share-card" class="relative overflow-hidden rounded-2xl border border-white/10 bg-gradient-to-br from-slate-900 via-indigo-950 to-slate-900 p-6 max-w-sm mx-auto">
            ...
            <div class="flex items-center justify-between bg-white/[0.03] rounded-xl p-3 border border-white/5 mb-4">
                <div class="text-center flex-1">
                    ...
                    <div class="text-sm text-white font-bold truncate">${safePlayerName}</div>
                ...
                <div class="text-center flex-1">
                    ...
                    <div class="text-sm text-white font-bold truncate">${safeEnemyName}</div>
...
export function getFarcasterShareUrl({ playerName, enemyName, playerWon, rounds }) {
    const safePlayerName = String(playerName || 'Unknown Fighter');
    const safeEnemyName = String(enemyName || 'Unknown Opponent');
    const result = playerWon ? 'VICTORY' : 'DEFEAT';
    const text = `${result}! ${safePlayerName} vs ${safeEnemyName} - ${rounds} rounds\n\nBattle your NFTs on Base Mint Arena`;
